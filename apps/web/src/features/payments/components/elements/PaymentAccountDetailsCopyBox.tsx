...
export default function PaymentAccountDetailsCopyBox({
  ...
  return (
    <div className="p-3 bg-brand-accent/5 rounded-2xl border border-brand-accent/20 space-y-2 min-w-0 max-w-full">
      ...
      <div className="space-y-1.5">
        {lines.map((line, index) => {
          ...
          return (
            <div
              key={index}
              className="flex items-center justify-between gap-2 text-xs text-foreground font-mono bg-background/80 px-3 py-2 rounded-xl border border-brand-accent/10 min-w-0 max-w-full"
            >
              <span className="truncate min-w-0">{line}</span>
              {numbersOnly && (
                <button
                  type="button"
                  onClick={() => copyNumber(numbersOnly, lineKey)}
                  className="shrink-0 inline-flex items-center gap-1 text-[10px] font-bold text-brand-accent hover:underline px-2.5 py-1 rounded-lg border border-brand-accent/20 bg-brand-accent/10 cursor-pointer"
                >
                  {isCopied ? (
                    <>
