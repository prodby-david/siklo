...
import CreateGroupFormFields from "../components/forms/CreateGroupFormFields";
import CreateGroupPreview from "../components/forms/CreateGroupPreview";
...
export default function CreateGroupUI() {
  const {
    register,
    errors,
    watch,
  ...
  if (!isUserLoading && !hasUsablePaymentAccount(user?.paymentAccounts)) {
    return (
      <main className="flex-1 bg-neutral-subtext/5 p-4 sm:p-6 md:p-10 min-h-screen flex items-center justify-center">
        <div className="text-center space-y-4 bg-background p-5 sm:p-8 rounded-2xl border border-neutral-border shadow-sm max-w-md w-full">
  ...
  return (
    <main className="flex-1 bg-neutral-subtext/5 p-4 sm:p-6 md:p-10">
