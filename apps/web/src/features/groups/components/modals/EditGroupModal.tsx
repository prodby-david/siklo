    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-1">
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2.5 border-t border-neutral-border/60 pt-3">
            <button
              type="button"
              onClick={onClose}
              className="w-full sm:w-auto cursor-pointer rounded-2xl border border-neutral-border px-4 py-2.5 text-xs font-bold text-foreground transition-all hover:bg-neutral-table-stripe active:scale-95"
            >
              Cancel
            </button>
            <button
              ...
              className="w-full sm:w-auto inline-flex justify-center cursor-pointer items-center gap-2 rounded-2xl bg-brand-accent px-5 py-2.5 text-xs font-extrabold text-brand-accent-foreground transition-all hover:bg-brand-accent-hover active:scale-95 disabled:opacity-50"
            >
              {isSubmitting ? (
                <>
