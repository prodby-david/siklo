...
export const createGroupSchema = baseCreateGroupSchema
  .extend({
    name: z
  ...
  .refine(
    (data) => {
      if (data.enableBackupFund) {
        return typeof data.backupFundPerTurn === "number" && data.backupFundPerTurn >= 1;
      }
      return true;
    },
    {
      message: "Please enter a reserve contribution of at least ₱1 per turn when backup fund is enabled",
      path: ["backupFundPerTurn"],
    }
  );
